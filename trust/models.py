...
class MeetingScheduler(models.Model):
    ...
    meeting_link = models.URLField(null = True, blank = True, validators = [URLValidator()])
    ...
    is_proceeding = models.BooleanField(default=False)

    def save(self, *args, **kwargs):
        current_datetime = timezone.now()

        if self.meeting_time_date < current_datetime.date() or \
            (self.meeting_time_date == current_datetime.date() and self.meeting_time_to < current_datetime.time()):
            self.is_proceeding = True                               #* the meeting has ended: true: PROCEEDINGS        
        else:
            self.is_proceeding = False                              #* the meeting hasn't ended: false: NOTICES
